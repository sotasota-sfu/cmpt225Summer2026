...
- [stack_demo.cpp](stack_demo.cpp): This example shows how to use an abstract
  ...
  implementations, one using a vector and one using a list. Both inherit from
  the same stack base class and so they share the same interface. The function
...
- [list_iterator_demo.cpp](list_iterator_demo.cpp): This example shows how to
  use an **iterator** to access the elements of a collection without needing
  access to the implementation details of the collection.
